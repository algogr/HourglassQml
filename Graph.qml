...
Item {
    ...
    function firstgraph()
    {
        ...
        from tasks_improved where year(starttime)>=year(curdate())-2  and taskcatid="+(chartcombo.currentIndex+1) +" group by YEAR(starttime),month(starttime)
        ...
        AND (`tasks_improved`.`taskcatid` ="+(chartcombo.currentIndex+1)+")) GROUP BY MONTH(`tasks_improved`.`starttime`) ");
    ...
    function seconfdgraph()
    {
        ...
        from tasks_improved where year(starttime)>=year(curdate())-4 and taskcatid="+(chartcombo.currentIndex+1) +" group by YEAR(starttime)
        order by sum(duration) desc")
        var maxindex1=querymodel.index(0,1)
        var max1=(querymodel.data(maxindex1))/60
        console.log("secondgrapgh:"+max1)
        ...
        AND (`tasks_improved`.`taskcatid` = "+(chartcombo.currentIndex+1)+")) GROUP BY YEAR(`tasks_improved`.`starttime`)");

        dataView1.barmax1=max1
        baraxis1.categories=[]
        for (var j=0;j<querymodel.rowCount();++j)
        {
            var ind=querymodel.index(j,0)
